...
pipeline {
    ...
    options {
        ansiColor('xterm')
        lock resource: "terraform_provider_acc_tests"
    }
            ...
            steps {
                withCredentials([
                    ...
                    sh'''#!/bin/bash
                        go version
                        export TF_LOG=info
                        go test -timeout 2400s -run '^(TestAccResAppVersions_app1|TestAccDefaultValues_app1|TestAccDefaultValuesRolling_app1|TestAccResApp_app1|TestAccResApp_Routes_updateToAndmore|TestAccResApp_dockerApp|TestAccResApp_dockerAppInvocationTimeout|TestAccResApp_app_bluegreen)$' github.com/terraform-providers/terraform-provider-cloudfoundry/cloudfoundry > acc_test.log
                        wc -l acc_test.log
                    '''
                
                    archiveArtifacts artifacts: "acc_test.log", fingerprint: true
                }
            }
        }
    }
